...
class Vg_intern extends Core_Model
{
     ...
     public function getlist()
     {
          ...
          /** get all victory interns in church */
          $sql = $this->vg_intern_script->getlist() . " WHERE userleader.churchid='$churchid'";
     ...
     public function set()
     {
          ...
          if ($value == '0') {
               $internid = isset($_POST['internid']) ? $_POST['internid'] : 0;
               if (empty($internid)) {
                    return array("success" => false, "info" => "Invalid internid");
               }
               $result = $this->delete('user_vg_intern', "internid=" . $internid);
          } else {
               $on2one = $this->one2one_get->getinfo($_POST['userid']);
               if ($on2one->leaderid > 0) {
                    $vgid = isset($_POST['vgid']) ? $_POST['vgid'] : 0;
                    if (empty($vgid)) {
                         return array("success" => false, "info" => "Invalid Victory Group");
                    }
                    $userid = isset($_POST['userid']) ? $_POST['userid'] : 0;
                    if (empty($userid)) {
                         return array("success" => false, "info" => "Invalid User");
                    }
