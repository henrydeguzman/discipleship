...
angular
	...
	.controller("main.header.controller", [
		"centralFctry",
		function(centralFctry) {
			var vm = this;
		}
	])
	.controller("main.login.controller", [
		"$scope",
		"centralFctry",
		function($scope, centralFctry) {
			var vm = this;
			$scope.form = {};
			$scope.required = {};
			...
			vm.verify = function(form) {
				...
				var posted = centralFctry.postData({
					url: "fetch/users_connection/verify",
					data: form
				});
	...
	.controller("main.resetpassword.controller", [
		"$scope",
		"centralFctry",
		"glfnc",
		function($scope, centralFctry, glfnc) {
			var vm = this;
			$scope.form = {};
			$scope.required = {};
			vm.recover = {};
			vm.recover.required = function() {
				$scope.required = { password: "required", confirm: "required" };
			};
			vm.recover.submit = function(userid, token) {
				//console.log('fired',userid,token);
				if (
					$scope.form.password === undefined ||
					$scope.form.confirm === undefined
				) {
					vm.recover.required();
				}
				if (
					$scope.form.password !== $scope.form.confirm &&
					$scope.form.password !== undefined &&
					$scope.form.confirm !== undefined &&
					glfnc.trim($scope.form.password) !== "" &&
					glfnc.trim($scope.form.confirm) !== ""
				) {
					vm.recover.success = false;
					vm.recover.info = "Confirm password does not match!";
					$scope.form.password = undefined;
					$scope.form.confirm = undefined;
					$scope.required = {};
					return;
				...
				var posted = centralFctry.postData({
					url: "fetch/users_connection/recover",
					data: $scope.form
				});
				if (posted.$$state !== undefined) {
					return posted.then(function(v) {
						...
						if (v.data.success) {
							location.assign(
								vtr.pathValue.base_url + "page/auth/reset-success"
							);
						} else {
							vm.recover.success = v.data.success;
							vm.recover.info = v.data.info;
							vm.recover.required();
						}
					});
				}
			...
			vm.verify.submit = function(form) {
				console.log(form);
				var posted = centralFctry.postData({
					url: "fetch/users_connection/reset_password",
					data: form
				});
				if (posted.$$state !== undefined) {
					return posted.then(function(v) {
						...
						if (!v.data.success) {
							required();
							vm.verify.info = v.data.info;
							vm.verify.color = "red";
						} else {
							location.assign(vtr.pathValue.base_url + "page/auth/link-sent");
						}
					});
				}
			};
